...
const openCollapse = () =>{
    ...
    if(btnCollapse[0].classList == "nav__collapseItem"){
        for(let i = 0;i<btnCollapse.length;i++){btnCollapse[i].classList.add('nav__collapse--active')}
        hiddenNav[0].style.display = "flex"
    }else{
        for(let i = 0;i<btnCollapse.length;i++){btnCollapse[i].classList.remove('nav__collapse--active')}
        ...
        return(
            <nav className="nav">
                <div className="g-container nav__container">
                    <Link className="nav__Link" to="/"><img className="nav__logo" src={logo}></img></Link>
                ...
                <Link to="/pl"><p  className="g-btn nav__lang">EU</p></Link>
            </nav>
        )
    }
